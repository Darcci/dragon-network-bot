...
from config import BOT_CONFIG, DISCORD_IDS, COLORS
...
# ====== VERIFICATION-SYSTEM FUNKTIONEN ======
def save_verification_message(message_id):
    """Speichert die Verification-Message ID"""
    if "verification" not in data:
        data["verification"] = {}
    data["verification"]["message_id"] = message_id
    dump()

def get_verification_message():
    """Gibt die gespeicherte Verification-Message ID zurück"""
    if "verification" not in data or "message_id" not in data["verification"]:
        return None
    return data["verification"]["message_id"]
